...
def calculate_location_score(location_info: str) -> str:
    ...
    for c in local_conflicts:
        ...
        conflicts.append(c)
    if amap_info and amap_info.get("is_restricted"):
        conflicts.append({"name": amap_info.get("reason") or "高德识别：水域/林地"})
    if conflicts:
        return f"❌ 该位置位于禁止区域'{conflicts[0]['name']}'内，综合评分：0分（不可选）"
...
def recommend_alternative_locations(location_info: str) -> str:
    ...
    if not candidates:
        return (
            f"当前数据库中评分>= {min_score} 的 POI 为 0，"
            f"已尝试限还到 {threshold} 仍不足。请适当调低期望评分。"
        )

    head = (
        f"根据评分阈值 min_score={min_score}（实际生效阈值={threshold}）筛选，"
        f"共找到 {len(candidates)} 个候选点位，评分范围 "
        f"{candidates[-1]['score']}-{candidates[0]['score']}。以下为前 {min(top_n, len(candidates))} 名：\n\n"
    )
    body = ""
...
💡 推荐高评分充电站的处理规则（重要！）：
- 当用户要求推荐某个评分以上的充电站（如"推荐一个 9.0 的"、"推荐评分 8 分以上的位置"）时：
  1. **必须** 调用 recommend_alternative_locations 工具，且**必须传入 min_score=X.X**。
     - 示例调用：工具输入 = '推荐,min_score=9.0'（全市范围评分≥9.0）
     - 示例调用：工具输入 = '26.0756,119.3034,min_score=8.5'（附近评分≥8.5）
  2. 工具返回的点位列表**必须原样保留名称、坐标、评分、理由**呈现给用户，不要丢失、不要“说找不到”。
  3. 如果工具明确返回“评分>=X 的为 0”才可以告诉用户找不到；只要返回中包含候选，就一定要完整列出。
  4. 数据库中确实存在 9.0、9.2、9.3、9.5、9.8 等多个高评分点位，不允许主观拒绝。
  5. 回复中使用 Markdown 表格或编号列表列出：名称 / 坐标 / 评分 / 推荐理由。
...
def quick_score_location(lat: float, lng: float) -> Dict[str, Any]:
    ...
    if conflicts:
        return {
            ...
            "nearby_pois": [],
            "nearby_roads": [],
            "message": f"该位置位于禁止区域：{conflicts[0]['name']}，不可选点",
        }
