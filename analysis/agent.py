...
def recommend_alternative_locations(location_info: str) -> str:
    ...
    推荐优质充电桩候选位置。
    输入格式：'纬度,经度' 例如 '26.0756,119.3034'
    也可以输入 '推荐' 或 '全市' 来获取全市范围内的高评分推荐。
    """
    from maps.models import POIData, TrafficFlow
    
    has_coords = False
    lat, lng = 26.0756, 119.3034  # 默认福州市中心
    try:
        parts = location_info.strip().split(',')
        if len(parts) >= 2:
            lat, lng = float(parts[0]), float(parts[1])
    ...
    # 获取所有高评分POI（降低阈值从8.0到7.0，确保更多结果）
    high_score_pois = POIData.objects.filter(ev_demand_score__gte=7.0).order_by('-ev_demand_score')
    
    candidates = []
    for poi in high_score_pois:
        dist = haversine_distance(lat, lng, poi.latitude, poi.longitude)
        # 有坐标时排除太近的（<0.1km），无坐标时不限制距离
    ...
    if not candidates:
        return "当前数据库中未找到符合条件的高评分候选位置。"
    
    result = f"基于POI和交通分析，推荐以下备选位置（共找到{len(candidates)}个候选）：\n\n"
    for i, c in enumerate(candidates[:8], 1):
        result += f"{i}. {c['name']}\n"
        result += f"   坐标：({c['lat']:.4f}, {c['lng']:.4f})\n"
        result += f"   充电需求评分：{c['score']}/10\n"
...
💡 推荐高评分充电站的处理规则：
- 当用户要求推荐某个评分以上的充电站（如"推荐一个9.0的"、"推荐评分8分以上的位置"）时：
  1. 必须调用 recommend_alternative_locations 工具获取高评分候选位置
  2. 如果用户没有指定坐标，传入 '推荐' 作为参数即可获取全市范围内的推荐
  3. 从返回结果中筛选出符合用户要求评分的候选，并给出详细推荐
  4. 数据库中有大量7.0-9.8分的高需求POI点位，一定能找到结果
  5. 在回复中列出具体的坐标、评分、推荐理由
