...
from maps.models import POIData, TrafficFlow, CandidateLocation, ExclusionZone
...
def run():
    ...
    # 更新主要水域的精确多边形边界（每次都执行，确保数据库始终有最新边界）
    WATER_POLYGONS = {
        "闽江（福州段）": {
            "type": "Polygon",
            "coordinates": [[
    ...
    for zone in ExclusionZone.objects.filter(zone_type='water'):
        ...
        if polygon:
            ...
            updated_zones += 1
    print(f"水域多边形边界更新完成：{updated_zones}个水域已写入精确多边形边界")
